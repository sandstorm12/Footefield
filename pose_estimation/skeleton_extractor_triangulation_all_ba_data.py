...
def calc_3d_skeleton(cameras, model_2d, cache):
    ...
    for idx_cam, camera in enumerate(cameras):
        ...
        # params = cv2.Rodrigues(extrinsics[:3, :3])[0].ravel().tolist() + list(extrinsics[:3, 3]) + [(mtx[0, 0] + mtx[0, 1]) / 2, dist[0, 0], dist[0, 1]]
        params = {
            'mtx': mtx,
            'dist': dist,
        ...
        # print(len(camera_params), len(camera_params[idx_cam]))
        # print(camera_params)
    ...
    # print(poses_global.reshape(-1, 3).shape)
...
if __name__ == "__main__":
    ...
    for experiment in data_loader.EXPERIMENTS.keys():
        ...
        camera_indices, \
        point_indices, \
        camera_params = calc_3d_skeleton(cameras, model_2d, cache)
        
        store_poses(poses_global,
                    'keypoints3d_{}'.format(experiment),
                    STORE_DIR)
        
        store_path = os.path.join(STORE_DIR, f"ba_{experiment}" + '.pkl')
        with open(store_path, 'wb') as handle:
            pickle.dump(
                {
