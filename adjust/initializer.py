...
def _generate_sh(dir_configs, dir_sh, configs):
    ...
    sh_path = os.path.join(dir_sh, "sh.sh")
    _store_sh(pattern, sh_path)

    return sh_path


def _generate_sh_master(dir_sh, sh_segment_list):
    pattern = "\n".join([f"sh \"{sh_path}\"" for sh_path in sh_segment_list])

    sh_path = os.path.join(dir_sh, "sh_master.sh")
    _store_sh(pattern, sh_path)
...
if __name__ == "__main__":
    ...
    for segment in range(num_segments):
        ...
        sh_path = _generate_sh(dir_configs_segment, dir_sh_segment, configs)
        sh_segment_list.append(sh_path)

    print("Generating sh master...")
    _generate_sh_master(dir_sh, sh_segment_list)
