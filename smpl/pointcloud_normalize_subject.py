...
DIR_OUTPUT = 'pointcloud_normalized'
...
def load_pointcloud(poses, subject, experiment, extrinsics, cache):
    pcd_np = []
    for idx in tqdm(range(len(poses))):
        ...
        pcd_np_instance = np.asarray(pcd_combined.points)
        pcd_np.append(pcd_np_instance)
...
def visualize_poses(poses, verts, faces, subject, pcds):
...
def load_smpl_params(file_org, subject):
    files_smpl = get_corresponding_files(file_org)

    idx_smpl = (subject + 1) % 2
    
    path_smpl = os.path.join(DIR_STORE, files_smpl[idx_smpl][0])
    with open(path_smpl, 'rb') as handle:
        smpl = pickle.load(handle)
    ...
    scale_1 = params['scale'] * scale_smpl
    translation_2 = params['translation']

    rotation_1 = np.linalg.inv(rotation)
...
def normalizer_pcs(pcd_np, transformation_1, rotation_1,
    ...
    return pcds_normalized


def store_smpl_pointcloud(pcds_normalized, experiment, subject):
    ...
    with open(path, 'wb') as handle:
        pickle.dump(pcds_normalized, handle,
...
# TODO: Shorten the main statement
